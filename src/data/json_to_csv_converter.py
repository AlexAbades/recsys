    ...
    with open(csv_file_path, "w+", newline="") as fout:
        ...
        with open(json_file_path) as fin:
            for line in fin:
                ...
                row_data = get_row(line_contents, column_names)
...
def get_nested_value(d, key):
    ...
    """
    if "." not in key:
        return d.get(key)  # Simplified to use .get() for conciseness
    base_key, sub_key = key.split(".", 1)
    sub_dict = d.get(
        base_key
    )  # Use .get() to avoid KeyError and return None if not found
    if sub_dict is None:  # Check if the sub-dictionary is None
        return None  # Cannot proceed further, so return None
    return get_nested_value(
        sub_dict, sub_key
    )  # Recursive call with the sub-dictionary and sub-key
